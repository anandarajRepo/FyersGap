"""
FyersGap — Gap Strategy State Machine.
...
STOP_FLAG_FILE = Path(".fyersgap_stop")
...
from services.fyers_data_service import FyersDataService
...
class GapStrategy:
    ...
    async def run(self) -> None:
        logger.info("=" * 60)
        logger.info("FyersGap Strategy starting | %s", now_ist().strftime("%Y-%m-%d"))
        logger.info("=" * 60)
        ...
        while True:
            if STOP_FLAG_FILE.exists():
                ...
                logger.info("FyersGap stopped gracefully.")
                return
