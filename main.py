"""
FyersGap — Entry point.

Usage
-----
  python main.py auth          Authenticate with Fyers (interactive OAuth flow)
  python main.py run           Start the gap trading strategy
  python main.py scan          One-shot gap scan (no orders placed)
  python main.py status        Show today's log tail
...
def cmd_run() -> None:
    ...
    strategy = GapStrategy(fyers)

    logger.info(
        "Starting FyersGap | DRY_RUN=%s | PORTFOLIO=RS.%.0f | MAX_POS=%d",
    ...
    try:
        ...
        logger.info("FyersGap stopped by user (Ctrl+C).")
