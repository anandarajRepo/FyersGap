"""
FyersGap — Configuration dataclasses loaded from environment variables.
...
@dataclass
class BrokerConfig:
    # App ID from the Fyers API portal, format: APPID-100
    app_id: str = field(default_factory=lambda: os.getenv("FYERS_APP_ID", ""))
    # Secret key from the Fyers API portal
...
@dataclass
class RiskConfig:
    portfolio_value: float = field(default_factory=lambda: _float("PORTFOLIO_VALUE", 6000000))
...
class PrevDayConditionConfig:
    ...
    # Bearish (SHORT) condition: close must be in the BOTTOM fraction.
